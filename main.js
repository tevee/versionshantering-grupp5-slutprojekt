...
import { alertWhenLogIn } from "./modules/amandaGrupp2.js"; //Amanda grupp 2
  ...
  getUserData("users", "")
    .then((users) => {
      ...
      if (createUser.username !== "" && createUser.password !== "") {
        ...
        alertWhenLogIn(
          `Welcome ${userNameInputValue}, you are now registered. Continue to log in page.`
        );
      }
    })
    .catch((error) => console.log(error));
...
logInFormEl.addEventListener("submit", (event) => {
  ...
  getUserData("users", "")
    .then((users) => {
      for (const user in users) {
        if (
          userNameInputValue === users[user].username &&
          passwordInputValue === users[user].password
        ) {
          ...
          displayDeleteBtnForUser();
          alertWhenLogIn(
            `Welcome ${userNameInputValue}, you are now logged in`
          );
          break;
        }
      }
...
publishMessageFormEl.addEventListener("submit", (event) => {
  ...
  const messageBoxColor = getComputedStyle(
...
messageBoardEl.addEventListener("click", (event) => {
  ...
  if (event.target.classList.contains("like-icon") && document.cookie !== "") {
    ...
    getUserData("messages", "").then((messages) => {
      for (const key in messages) {
        ...
        if (key === parentContainer.id && !userList.includes(loggedInUser)) {
          ...
          putData("messages", likes, key, "likes")
            .then((likesObj) => {
              const likesEl = document.querySelector(
                `#${key} > .message-footer > .amount-of-likes`
              );
          ...
          key === parentContainer.id &&
          userList.includes(loggedInUser)
        ) {
          const likeIcon = document.querySelector(
            `#${key} > .message-footer > .like-btn > i`
          );
